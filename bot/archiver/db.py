...
async def save_message(db, msg):
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            # If this is a GM post, mark the channel as having GM posts
            # This denormalization speeds up channel filtering queries
            is_gm_query = await conn.fetchval(
                "SELECT COALESCE((is_gm)::text, '0') IN ('1','t','true') FROM members WHERE member_id = $1",
                str(msg.author.id)
            )
            if is_gm_query:
